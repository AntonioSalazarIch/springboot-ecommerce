...
public class ProductServiceImpl implements ProductService {
  ...
  @Override
  public Product updateById( UUID id, ProductDto productDto ) {
    Product productNow = productRepository.findById(id)
            .orElseThrow(() -> new EntityNotFoundException("Product", id));

    productNow.setName( productDto.getName() );
    productNow.setDescription( productDto.getDescription() );
    productNow.setActive( productDto.isActive() );
    productNow.setPrice( productDto.getPrice() );
    productNow.setStock( productDto.getStock() );
    productNow.setImageUrl( productDto.getImageUrl() );
    return productRepository.save( productNow );
  }
